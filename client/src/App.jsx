...
function App() {
  ...
  const submit_description = () => {
    ...
    axios
      .post("http://88.99.162.157:5050/test", data)
    ...
    if (selectedFile) {
      ...
      try {
        const response = await axios.post(
          "http://88.99.162.157:5050/test/upload",
          formData,
          {
            headers: {
  ...
  const submit_candidate = () => {
    ...
    //communication with backend
    axios
      .post("http://88.99.162.157:5050/test/candidateInfo", candidate_data)
      .then((res) => console.log(res.data.confirm))
      .catch(() => console.log("error"));
  ...
  const sendToBot = async () => {
    console.log(
      ...
      console.log("select level status is", levelStatus)
    const sendData = {
    ...
    const url = "http://88.99.162.157:5000/screen_start";
